...
Step B에서 각 reply의 `files` 배열을 순회해 `image/*` mimetype인 첫 번째 파일 URL을 `user_images` dict에 저장. 이미지 다운로드는 `img_headers`(Content-Type 없는 인증 전용 헤더) 사용 — 공통 `headers`에 `Content-Type: application/x-www-form-urlencoded`가 있어 바이너리 다운로드 시 오류 발생.
